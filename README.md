# Repository rules

## About

Checkstyle, detekt, Dart Analyzer, CodeNarc, and Rubocop rules, Commit Convention, .gitignore templates and GitHub rules.

## Rules

* [Checkstyle (without custom checks)](./rules/java/checkstyle/checkstyle.xml)
* [Checkstyle Custom (with custom Java checks)](./rules/java/checkstyle-custom/checkstyle.xml)
* [Checkstyle Android (with custom Java + Android checks)](./rules/java/android/checkstyle.xml)
* [detekt](./rules/kotlin/detekt/detekt.yml)
* [Dart Analyzer](./rules/flutter/dart_analyzer/analysis_options.yaml)
* [CodeNarc](./rules/groovy/codenarc/codenarc.groovy)
* [RuboCop](./rules/ruby/rubocop/.rubocop.yml)

## Conventions

* [Commit Convention](./commit-convention/COMMIT_CONVENTION.md)

## .gitignore files

* [Android](./gitignore/android/.gitignore)
* [.NET](./gitignore/dotnet/.gitignore)
* [Dart](./gitignore/dart/.gitignore)
* [Flutter](./gitignore/flutter/.gitignore)
* [Groovy](./gitignore/groovy/.gitignore)
* [HTML](./gitignore/html/.gitignore)
* [Java](./gitignore/java/.gitignore)
* [Kotlin](./gitignore/kotlin/.gitignore)
* [NodeJS](./gitignore/nodejs/.gitignore)
* [Python](./gitignore/python/.gitignore)
* [Ruby](./gitignore/ruby/.gitignore)

## Repositories

* [GitHub](./github)
